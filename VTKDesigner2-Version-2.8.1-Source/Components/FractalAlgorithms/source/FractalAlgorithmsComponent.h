...
#ifndef FRACTALALGORITHMS_COMPONENT_H
#define FRACTALALGORITHMS_COMPONENT_H
...
class FractalAlgorithmsComponent : public GCF::AbstractComponent,
                                 virtual public IVisSystemNodeFactory,
                                 virtual public IAboutData
{
    Q_OBJECT
    Q_INTERFACES(IVisSystemNodeFactory IAboutData)
    GCF_DECLARE_COMPONENT(FractalAlgorithmsComponent)

public:
    static FractalAlgorithmsComponent & instance();
    ~FractalAlgorithmsComponent();
...
private:
    FractalAlgorithmsComponentData* d;
};
