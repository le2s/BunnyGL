...
#ifndef CVTKIMAGEEXTRACTCOMPONENTS_VIS_NODE_H
#define CVTKIMAGEEXTRACTCOMPONENTS_VIS_NODE_H
...
class CVtkImageExtractComponents : public CVtkThreadedImageAlgorithm
#ifdef ENABLE_ADVANCED_PROPERTIES
...
public:
    CVtkImageExtractComponents();
    ~CVtkImageExtractComponents();

    Q_PROPERTY(QList<QVariant> Components READ components WRITE setComponents)
    void setComponents(const QList<QVariant> & com);
    QList<QVariant> components() const;

    Q_INVOKABLE int numberOfComponents();
...
private:
    CVtkImageExtractComponentsData* d;
};
