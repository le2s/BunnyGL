...
#ifndef IMAGINGALGORITHMS_COMPONENT_H
#define IMAGINGALGORITHMS_COMPONENT_H
...
class ImagingAlgorithmsComponent : public GCF::AbstractComponent,
                                 virtual public IVisSystemNodeFactory,
                                 virtual public IAboutData
{
    Q_OBJECT
    Q_INTERFACES(IVisSystemNodeFactory IAboutData)
    GCF_DECLARE_COMPONENT(ImagingAlgorithmsComponent)

public:
    static ImagingAlgorithmsComponent & instance();
    ~ImagingAlgorithmsComponent();
...
private:
    ImagingAlgorithmsComponentData* d;
};
