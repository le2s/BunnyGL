...
GCF::Components::MainWindowComponent::MainWindowComponent()
{
    ...
    d->mainWindow->statusBar()->showMessage("Built using Generic Component Framework from VCreate Logic (http://www.vcreatelogic.com/products/gcf)");
...
void GCF::Components::MainWindowComponent::setApplicationIcon(const QString& appIcon)
{
    ...
    qApp->setWindowIcon( QIcon(d->appIcon) );
    if(d->mainWindow)
        d->mainWindow->setWindowIcon(qApp->windowIcon());
}
...
{
#if 0
    QByteArray ba = d->mainWindow->saveState();
    settings()["MainWindowState"] = ba;
#endif
}
...
void GCF::Components::MainWindowComponent::componentCreated(GCF::AbstractComponent* comp)
{
    ...
    if( comp )
    {
        ...
        QString name;
        if(gui.hasKey("title"))
            name = gui["title"].toString();
        else
            name = comp->componentName();
        d->splashScreen->showMessage( name );
    }
    else
    ...
    {
        QTimer::singleShot(1000, d->splashScreen, SLOT(deleteLater()));
#if 0
        if(settings().hasKey("MainWindowState"))
        {
            QByteArray ba = settings()["MainWindowState"].toByteArray();
            d->mainWindow->restoreState(ba);
        }
#endif
    }
}
