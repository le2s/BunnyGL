...
bool GCF::Components::Project::newProject()
{
    ...
    QTimer::singleShot(100, this, SLOT(slotUnmodified()));
...
void GCF::Components::Project::slotOpenRecentFile()
{
    ...
    QMetaObject::invokeMethod(this, 
            "currentProjectFileNameChanged", 
            Qt::QueuedConnection, 
            Q_ARG(QString, d->projectFileName));
}

void GCF::Components::Project::slotUnmodified()
{
    /*
    If you cant win the game, change the rules!

    d->modified magically becomes true after a call to loadProject(),
    saveProject() and newProject(). Though this doesnt happen all
    the time - it is still visible and irritating.

    Debugging didnt do much help, because it is a maniac effort to
    figure out why d->modified becomes true. Also, we noticed that
    some GCF applications have this issue; while others dont.

    To fix this problem; we have now written this slot. It is
    sceduled to be called 100ms after load/save/new/close. This
    slot ensures that d->modified becomes false no-matter-what.
    */
    if(!d->modified)
        return;
