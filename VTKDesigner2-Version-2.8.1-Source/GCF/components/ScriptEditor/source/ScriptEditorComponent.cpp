...
{
    ScriptEditorComponentData() : scriptEditor(0), workspaceTabWidget(0),
        soExplorer(0), xmlUtils(0), configPageSheet(0) { }
...
{
    qDebug("This function is not supported right now");
    Q_UNUSED(actionName);
    Q_UNUSED(text);
    Q_UNUSED(iconName);
    Q_UNUSED(rowSpan);
...
void GCF::Components::ScriptEditorComponent::finalizeComponent()
{
    ...
    settings()["TabSpace"] = d->scriptEditor->tabSpace();
    settings()["FontSize"] = d->scriptEditor->fontSize();
    settings()["FontName"] = d->scriptEditor->fontName();
}
...
void GCF::Components::ScriptEditorComponent::creationAnnounced()
{
...

    {
        ...
        if(d->workspaceTabWidget && d->workspaceTabWidget->indexOf(d->scriptEditor) >= 0)
            setActive(d->workspaceTabWidget->currentWidget() == d->scriptEditor);
        else
            setActive(true);
    }
    ...
    if(settings().hasKey("TabSpace"))
        d->scriptEditor->setTabSpace( settings()["TabSpace"].toInt() );
    if(settings().hasKey("FontSize"))
        d->scriptEditor->setFontSize( settings()["FontSize"].toInt() );
    if(settings().hasKey("FontName"))
        d->scriptEditor->setFontName( settings()["FontName"].toString() );
}
...
{
    return QIcon(":/ScriptEditor/scriptingenv.png");
}
...
void GCF::Components::ScriptEditorComponent::addLoadFormContainer(QWidget* widget, const QString& completeName)
{
    if(!widget)
        return;
    ...
    int index = d->widgetList.indexOf(widget);
    if(index >= 0)
        d->widgetNames[index] = completeName;
    else
    {
        d->widgetList.prepend(widget);
        d->widgetNames.prepend(completeName);
    }
}

void GCF::Components::ScriptEditorComponent::removeLoadFormContainer(QWidget* widget)
{
    if(!widget)
        return;
...
void GCF::Components::ScriptEditorComponent::changeLoadFormContainer(QWidget* widget, const QString& completeName)
{
    if(!widget)
        return;
...
int GCF::Components::ScriptEditorComponent::loadFormContainerCount() const
{
    return d->widgetList.count();
}

QWidget* GCF::Components::ScriptEditorComponent::loadFormContainerWidgetAt(int index) const
{
...
QString GCF::Components::ScriptEditorComponent::loadFormContainerNameAt(int index) const
{
    if(index < 0 || index >= d->widgetList.count())
        return QString();

    return d->widgetNames.at(index);
}
