...
void GCF::Components::ScriptableObjectExplorer::updateExplorer(bool updateOldObjects)
{
    ...
    for(int i=0; i<d->scriptEngineManager->scriptableObjectCount(); i++)
    {
        ...
        if(oldObjects.contains(object))
        {
            ...
            {
                QTreeWidgetItemIterator it(objItem);
                while(*it)
                {
