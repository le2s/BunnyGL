...
{
    return "http://www.vcreatelogic.com/support/features/";
}
...
{
    // Do nothing
    this->setComponentName("UtilsComponent");
    this->setGuiXmlFileName(":/UtilsComponent/GUIXML");
}
...
QColor GCF::Components::UtilsComponent::randomColor(const QPair<qreal,qreal>& redRange, const QPair<qreal,qreal>& greenRange,
                                                    const QPair<qreal,qreal>& blueRange, const QPair<qreal,qreal>& alphaRange)
{
    qreal r = redRange.first == redRange.second ? redRange.first : this->randomNumber(redRange.first, redRange.second);
    qreal g = redRange.first == redRange.second ? redRange.first : this->randomNumber(greenRange.first, greenRange.second);
    qreal b = blueRange.first == blueRange.second ? blueRange.first : this->randomNumber(blueRange.first, blueRange.second);
    qreal a = alphaRange.first == alphaRange.second ? alphaRange.first : this->randomNumber(alphaRange.first, alphaRange.second);
...
QColor GCF::Components::UtilsComponent::interpolatedColor(qreal value, const QColor& start, const QColor& end)
{
    if(value < 0)
        value = 0;
    ...
    qreal r = (end.redF() - start.redF())*value + start.redF();
    qreal g = (end.greenF() - start.greenF())*value + start.greenF();
    qreal b = (end.blueF() - start.blueF())*value + start.blueF();
    qreal a = (end.alphaF() - start.alphaF())*value + start.alphaF();
...
QString GCF::Components::UtilsComponent::findFileInPath(const QString& fileName)
{
    return GCF::Application::findFileInPath(fileName);
}

QStringList GCF::Components::UtilsComponent::findFilesInPath(const QString& fileName)
{
    return GCF::Application::findFilesInPath(fileName);
}

void GCF::Components::UtilsComponent::deleteDirectory(const QString& dir, bool deleteDir)
{
    GCF::Application::deleteDirectory(dir, deleteDir);
}
