...
#include "IMessageEncoderDecoder.h"
...
public:
    ...
    Q_INVOKABLE QString accessPointName() const;
    ...
    // These functions make GCF 2.4+ binary incompatible with
    // GCF 2.3-.
    void setAllowAllApplications(bool val);
    bool isAllowAllApplications() const;

    void addAllowedApplication(const QString& name);
    void removeAllowedApplication(const QString& name);
    QStringList allowedApplications() const;
    bool isApplicationAllowed(const QString& name) const;
    ...
    // List of message encoder-decoders
    void registerMessageEncoderDecoder(IMessageEncoderDecoder* encDec);
    void unregisterMessageEncoderDecoder(IMessageEncoderDecoder* encDec);
    int messageEncoderDecoderCount() const;
    IMessageEncoderDecoder* messageEncoderDecoderAt(int index) const;
    int indexOfMessageEncoderDecoder(IMessageEncoderDecoder* encDec) const;

    // For making async invoke method calls.
    GCF::Message* handleMessage(GCF::Message* message) const;
    IMessageEncoderDecoder* messageEncoderDecoderFor(const QByteArray& ba) const;
...
public:
    ...
    bool isAllowObjectAccess(AbstractComponent* comp, QObject* object, RemoteApp* remoteApp) const;
    bool isAllowObjectMethodInvoke(AbstractComponent* comp, QObject* object, int methodIdx, RemoteApp* remoteApp) const;
    bool isAllowObjectSetProperty(AbstractComponent* comp, QObject* object, int propIdx, RemoteApp* remoteApp) const;
    bool isAllowObjectGetProperty(AbstractComponent* comp, QObject* object, int propIdx, RemoteApp* remoteApp) const;
    bool isAllowObjectSignalConnect(AbstractComponent* comp, QObject* object, int signalIdx, RemoteApp* remoteApp) const;
    bool isAllowObjectSignalDelivery(AbstractComponent* comp, QObject* object, int signalIdx, RemoteApp* remoteApp) const;
