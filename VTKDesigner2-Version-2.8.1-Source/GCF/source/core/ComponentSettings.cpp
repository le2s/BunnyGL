...
settings are stored in a file that has the following name format \c {ComponentName}Settings.xml.
For example, the component settings for ProjectManager can be found in ProjectManagerSettings.xml.

The location of settings file depends on the platform on which you are using GCF.

- On Windows: settings files are stored into \c %APPDATA%/{OrganizationName}/{ApplicationName}
- On Linux: settings files are stored into <code> $HOME/.{OrganizationName}/{ApplicationName} </code>

Where

- {OrganizationName} is the value returned by \c gApp->organizationName(). If no organization name is set
then \c VCreateLogic is used by default.
- {ApplicationName} is the value returned by \c gApp->applicationName()
