...
/**
\class GCF::RemoteAppDiscovery GCF/RemoteAppDiscovery
\ingroup gcfipc
...
void GCF::RemoteAppDiscovery::readBroadcastDatagram()
{
    while(d->broadcastSocket.hasPendingDatagrams())
    {
        ...
        {
            if( d->knownRAPList[i].Port == rapInfo.Port )
            {
                if( d->knownRAPList[i].Address == rapInfo.Address )
                {
                    alreadyDiscovered = true;
                    break;
                }

                if( d->isLocalHostAddress(d->knownRAPList[i].Address) && d->isLocalHostAddress(rapInfo.Address) )
                {
                    alreadyDiscovered = true;
                    break;
                }
        ...
        qDebug() << "Address: " << rapInfo.Address.toString() << " - " << rapInfo.Port;
...
/**
\fn void GCF::RemoteAppDiscovery::discoveredRemoteApp(const QHostAddress& addr, int port);

This signal is emitted when a remote application was discovered at \c addr at \c port. This
signal is emitted even before a connection attempt is made to the remote application.
Immediately after the emission of this signal, the \ref GCF::RemoteAppDiscovery class makes
an attempt to establish connection.
*/

/**
\fn void GCF::RemoteAppDiscovery::discoveredRemoteApp(GCF::RemoteAppAccessPoint* rap);

This signal is emitted when a remote application, referred to by \c rap, was discovered. This
signal is made after connection to the remote application has been established.
*/
