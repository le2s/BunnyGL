...
#include <GCF/GenericSignalSpy>
#include <QMetaObject>
#include <QMetaMethod>

/**
\class GCF::GenericSignalSpy GCF/GenericSignalSpy
                     ...
                     :QObject(parent)
{
    ...
    // Perform a dummy signal-slot connection
    QObject::connect(d->sender, signal, this, SLOT(dummySlot()));
}
...
GCF::GenericSignalSpy::~GenericSignalSpy()
{
    ...
    d->signal.prepend('2');
    QObject::disconnect(d->sender, d->signal.constData(), this, SLOT(dummySlot()));
}
...
Spied signals are connected to this slot so that the object that is being spied
on gets a connectNotify() and disconnectNotify() call.
*/
void GCF::GenericSignalSpy::dummySlot()
{
    // Do nothing
}
